...
import "./data.css";
import { games } from "./gamesData.js"; // shared data file

const GameCard = ({ game }) => {
  const navigate = useNavigate();
  return (
    <div className="card" onClick={() => navigate(`/game/${game.id}`)} style={{ cursor: "pointer" }}>
      <div className="imageWrapper">
        <img src={game.image} alt={game.title} className="image" />
      </div>
      <div className="details">
...
const Data = () => {
  ...
  return (
    <div className="appContainer">
      <h2 className="sectionTitle">Popular Games</h2>
      <div className="carouselNav">
        <button onClick={handlePrev} disabled={startIndex === 0} className="navBtn">❮</button>
        <div className="carouselWrapper">
          <div className="carousel">
            {games.slice(startIndex, startIndex + visibleCards).map((game) => (
              <GameCard key={game.id} game={game} />
